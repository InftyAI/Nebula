...
// withAttribution swaps in a label set for one test and restores the default afterwards. It goes
// through configureCost rather than InitCost on purpose: a registry refuses the second set of label
// dimensions it ever sees for a name, and testutil collects straight from the collector anyway.
func withAttribution(t *testing.T, names ...string) {
	t.Helper()
...
func TestParseCostLabels(t *testing.T) {
	cases := map[string]struct {
		...
		"empty is no attribution": {spec: "", want: nil},
		"names in order":          {spec: "org_id,team_id", want: []string{"org_id", "team_id"}},
		"whitespace and blanks":   {spec: " org_id , , team_id ", want: []string{"org_id", "team_id"}},
		// One token has to be legal as a Pod label key AND a Prometheus label name.
		"hyphen is not a prometheus name": {spec: "org-id", err: true},
		"dot is not a prometheus name":    {spec: "org.id", err: true},
		"qualified key":                   {spec: "nebula.inftyai.com/org-id", err: true},
		"leading digit":                   {spec: "2team", err: true},
		"trailing underscore":             {spec: "org_", err: true},
		"duplicate":                       {spec: "org_id,org_id", err: true},
	}
...
// A name that shadows one of the counter's own dimensions is not rejected by the parser — it is
// caught one step later, when InitCost registers a descriptor with a duplicate label name. This
// pins that remaining net, because it is the only thing standing between --cost-labels=provider and
// a manager that starts with a broken cost metric.
func TestInitCost_RejectsAShadowingName(t *testing.T) {
	t.Cleanup(func() { configureCost(nil) })

	if err := InitCost([]string{"provider"}); err == nil {
		t.Fatal("InitCost accepted a label name the counter already carries")
	}
}

// The configured names become metric labels, and a Pod that carried none of them is reported as
// "none" rather than dropping the dimension — which would silently split the series.
func TestRecordWindow_Attribution(t *testing.T) {
	withAttribution(t, "org_id", "team_id")

	l := Labels{Provider: "modal", Accelerator: "H100", AcceleratorCount: 1}
	RecordWindow(l, "Bound", map[string]string{"org_id": "acme", "team_id": "ml"}, 3.95)
	RecordWindow(l, "Bound", map[string]string{"org_id": "acme"}, 1.05)
...
// Values are looked up BY NAME, so the order they were stamped in cannot slide a team_id into the
// org_id column — the property that makes a --cost-labels change safe.
func TestRecordWindow_AttributionIsKeyedNotPositional(t *testing.T) {
	withAttribution(t, "org_id", "team_id")

	l := Labels{Provider: "modal"}
	RecordWindow(l, "Bound", map[string]string{"team_id": "ml", "org_id": "acme"}, 1)
	RecordWindow(l, "Bound", map[string]string{"org_id": "acme", "team_id": "ml"}, 1)
