...
func TestLabels_RendersInProvisionLabelOrder(t *testing.T) {
	tests := []struct {
		name string
		in   Labels
		want []string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := tt.in.values()
			if len(got) != len(provisionLabels) {
				t.Fatalf("values() = %v (%d values), want %d to match provisionLabels %v",
					got, len(got), len(provisionLabels), provisionLabels)
			}
			for i := range tt.want {
				if got[i] != tt.want[i] {
					t.Fatalf("values()[%d] (%s) = %q, want %q", i, provisionLabels[i], got[i], tt.want[i])
				}
			}
		})
	}
