...
func (s SandboxSpec) String() string {
	return fmt.Sprintf("SandboxSpec{Image:%s Command:%v Args:%v Env:%s GPU:%s GPUCount:%d CPU:%g "+
		...
		s.Image, s.Command, s.Args, provider.RedactedEnv(s.Env), s.GPU, s.GPUCount, s.CPU,
