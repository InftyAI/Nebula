...
type NodeClaimStatus struct {
	...
	// label key, verbatim — so this map reads like the Pod it came from, including any domain
	// prefix. The metric emits under a derived name Prometheus accepts
	// ("example.com/org-id" here is org_id there), so do not expect the two to match by string.
	//
