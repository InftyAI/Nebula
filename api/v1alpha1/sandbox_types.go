...
// shell, a scratch GPU machine.
//
// A Sandbox is SINGULAR — one object, one instance, no replicas field. A sandbox is
// not fungible: someone is attached to it, it holds state in its filesystem, and its
// name is its stable identity. A rolling update would evict a live session, and
// "scale in by one" would have to guess whose box to kill. The count lives one level
// up in SandboxSet, which creates N Sandbox OBJECTS — that is what keeps per-box
// RBAC, image, TTL, and a visible failure working underneath a pool.
//
// It reuses corev1 types (ResourceRequirements, EnvVar) because the controller
// synthesizes a Pod, so the spec must be PodSpec-shaped anyway. Re-declaring them
// would fork the source of truth for the accelerator COUNT, which placement and the
// scheduler both read from the container's nvidia.com/gpu limit.
//
// The CEL rule below rejects a GPU count with no accelerator type: that pair is
// contradictory (util.AcceleratorRequest errors on it), so without the rule the
// object is admitted and then fails at PLACEMENT minutes later. The inverse is
// allowed — a type with no count means one accelerator.
