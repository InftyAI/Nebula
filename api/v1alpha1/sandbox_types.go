...
//
// The count lives one level up, in SandboxPool, which keeps N Sandboxes warm and
// owns /scale so `kubectl scale` and HPA work. That split is what lets this type
// stay singular: because the pool creates Sandbox OBJECTS rather than replicas
...
type SandboxSpec struct {
	...
	//
	// It has no command, and setting one is rejected at admission. That is not a
	// simplification, it is the process model: SandD runs as PID 1 in the container
	// and spawns the workload as its child so it owns the workload's stdout/stderr
	// pipes, which is what makes `kubectl logs` possible against an instance in
	// another cloud. A user-supplied command would displace SandD and take the log
	// and exec paths with it. An interactive sandbox has nothing to run at boot
	// anyway — the whole point is that commands arrive later, over exec.
	// +kubebuilder:validation:MinLength=1
